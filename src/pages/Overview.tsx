  ...
  return (
    <div className="page-stack overview-page">
        ...
        {TOPICS.map((topic) => {
          ...
          return (
            <article className="overview-topic-card" key={topic.key} style={{ "--topic-color": topic.color } as CSSProperties}>
              ...
              {item.ready ? <>
                <div className="overview-topic-card__value"><small>{topic.key === "fixedIncome" ? "Valor líquido a receber" : "Valor atual"}</small><strong>{topic.currency === "USD" ? formatCurrency(item.value) : formatBrl(item.value)}</strong></div>
      ...
      {allocation.length > 0 && <section className="panel overview-allocation">
        <header className="panel__header"><div><h2>Distribuição entre as classes</h2><p>Participação no patrimônio consolidado em dólar</p></div></header>
