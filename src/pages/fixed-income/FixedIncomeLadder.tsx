...
export function FixedIncomeLadder({ model, usdRate }: { model: FixedIncomeModel; usdRate: number | null }) {
  ...
  return <div className="page-stack">
    <section className="metrics-grid metrics-grid--three">
      ...
      <MetricCard label={`Total a receber em ${activeYear.year}`} value={formatBrl(activeYear.metrics.netAmount)} secondaryValue={formatUsdFromBrl(activeYear.metrics.netAmount, usdRate)} icon={<CircleDollarSign size={19} />} helper={`${activeYear.metrics.assetCount} vencimentos no ano`} accent="violet" />
    </section>
    <Section title={`Escada de vencimentos de ${activeYear.year}`} subtitle="Selecione o ano para conferir os ativos existentes e os meses que ainda precisam de compra" action={<FixedIncomeYearSelector years={model.years.map(({ year }) => year)} value={activeYear.year} onChange={setSelectedYear} />}>
      <div className="maturity-ladder">{activeYear.months.map((month) => <article key={month.month} className={`maturity-month ${month.covered ? "maturity-month--covered" : "maturity-month--missing"}`}>
        ...
        <footer><span><CircleDollarSign size={14} /><span>A receber <strong>{formatBrl(month.amountToReceive)}</strong><small className="currency-conversion">{formatUsdFromBrl(month.amountToReceive, usdRate)}</small></span></span><span><TrendingUp size={14} /><span>Lucro <strong>{formatBrl(month.profit)}</strong><small className="currency-conversion">{formatUsdFromBrl(month.profit, usdRate)}</small></span></span></footer>
      </article>)}</div>
    </Section>
  </div>;
}
