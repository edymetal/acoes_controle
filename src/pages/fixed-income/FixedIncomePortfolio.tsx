...
function getFixedIncomeSortValue(
  ...
  if (key === "totalDays") {
    return calculateFixedIncomeTermDays(investment.purchaseDate, investment.maturityDate);
  }
...
export function FixedIncomePortfolio({ model, usdRate }: { model: FixedIncomeModel; usdRate: number | null }) {
  ...
  return <div className="page-stack">
    <section className="metrics-grid metrics-grid--three">
      <MetricCard label="Principal aplicado" value={formatBrl(model.metrics.currentPrincipal)} secondaryValue={formatUsdFromBrl(model.metrics.currentPrincipal, usdRate)} icon={<PiggyBank size={19} />} helper={`${model.metrics.assetCount} ativos · proxy atual`} accent="blue" />
      <MetricCard label="Líquido projetado" value={formatBrl(model.metrics.projectedNetAmount)} secondaryValue={formatUsdFromBrl(model.metrics.projectedNetAmount, usdRate)} icon={<CircleDollarSign size={19} />} helper="No vencimento, após o IR informado" accent="violet" />
      <MetricCard label="Lucro projetado" value={formatBrl(model.metrics.projectedProfit)} secondaryValue={formatUsdFromBrl(model.metrics.projectedProfit, usdRate)} icon={<TrendingUp size={19} />} helper={formatPercent(model.metrics.projectedReturnRate)} change={model.metrics.projectedProfit} accent="green" />
    </section>
    <Section title="Ativos de renda fixa" subtitle="Vencimento, recebimento e lucro de cada aplicação">
      ...
      {investments.length ? <div className="table-wrap table-wrap--fixed-income"><table className="fixed-income-table"><thead><tr>
        ...
        <SortableHeader sortKey="totalDays" sortConfig={sortConfig} onSort={requestSort}>Total de dias</SortableHeader>
