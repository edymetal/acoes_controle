...
export function FixedIncomePortfolio({ model, usdRate }: { model: FixedIncomeModel; usdRate: number | null }) {
  ...
  return <div className="page-stack">
    ...
    <Section title="Ativos de renda fixa" subtitle="Vencimento, recebimento e lucro de cada aplicação">
      ...
      {investments.length ? <div className="table-wrap"><table className="fixed-income-table"><thead><tr>
      ...
      </tr></thead><tbody>{sortedInvestments.map((item) => <tr key={item.id}><td><div className="asset-cell"><span className="ticker-avatar ticker-avatar--fixed-income">{item.type.slice(0, 2)}</span><span><strong>{item.name}</strong><small>{item.type} · risco {item.risk ?? "—"}</small></span></div></td><td><strong>{formatYield(item.yield)}</strong></td><td><span className="dual-value"><strong>{formatDate(item.purchaseDate)}</strong><small>{formatBrl(item.investedAmount)} · <span className="currency-conversion">{formatUsdFromBrl(item.investedAmount, usdRate)}</span></small></span></td><td><span className="date-emphasis"><CalendarClock size={14} />{formatDate(item.maturityDate)}</span></td><td><span className="dual-value"><strong>{formatBrl(item.investedAmount)}</strong><small className="currency-conversion">{formatUsdFromBrl(item.investedAmount, usdRate)}</small></span></td><td><span className="dual-value"><strong>{formatBrl(item.netAmount)}</strong><small className="currency-conversion">{formatUsdFromBrl(item.netAmount, usdRate)}</small></span></td><td><Value value={item.profit}><span className="dual-value"><strong>{formatBrl(item.profit)}</strong><small className="currency-conversion">{formatUsdFromBrl(item.profit, usdRate)}</small></span></Value></td><td>{item.fgcGuarantee ? <span className="fgc-badge"><ShieldCheck size={14} /> FGC</span> : <span className="muted-badge">Sem FGC</span>}</td></tr>)}</tbody></table></div> : <EmptyState title="Nenhum ativo encontrado" description="Ajuste a busca para visualizar outras aplicações." />}
    </Section>
  </div>;
}
