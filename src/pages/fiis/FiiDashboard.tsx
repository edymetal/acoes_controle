...
import { formatBrl, formatNumber, formatPercent, formatTransactionDate, formatUsdFromBrl } from "../../lib/format";
  ...
  return (
    <div className="page-stack">
      ...
      <section className="metrics-grid">
        ...
        <MetricCard label="Lucro realizado" value={accountingComplete ? formatBrl(metrics.realizedProfit) : "Indisponível"} secondaryValue={accountingComplete ? formatUsdFromBrl(metrics.realizedProfit, usdRate) : undefined} icon={<BadgeDollarSign size={19} />} helper={accountingComplete ? "Em cotas vendidas" : "Ordem das operações ambígua"} change={accountingComplete ? metrics.realizedProfit : undefined} accent="green" />
        <MetricCard label="Resultado em aberto" value={financialResultsComplete ? formatBrl(metrics.unrealizedProfit) : "Indisponível"} secondaryValue={financialResultsComplete ? formatUsdFromBrl(metrics.unrealizedProfit, usdRate) : undefined} icon={<TrendingUp size={19} />} helper={financialResultsComplete ? formatPercent(metrics.openReturn) : "Base financeira incompleta"} change={financialResultsComplete ? metrics.unrealizedProfit : undefined} accent="violet" />
        <MetricCard label="Resultado total" value={financialResultsComplete ? formatBrl(metrics.totalProfit) : "Indisponível"} secondaryValue={financialResultsComplete ? formatUsdFromBrl(metrics.totalProfit, usdRate) : undefined} icon={<CircleDollarSign size={19} />} helper={financialResultsComplete ? `${formatPercent(metrics.totalReturnOnPurchases)} sobre compras` : "Base financeira incompleta"} change={financialResultsComplete ? metrics.totalProfit : undefined} accent="amber" />
      </section>
      ...
      <section className="dashboard-grid dashboard-grid--lower">
        ...
        <Section title="Movimentações recentes" subtitle={`${transactions.length} registros de FIIs processados`} sensitiveSubtitle action={<button className="text-button" type="button" onClick={() => onNavigate("fii-history")}>Ver histórico <ArrowRight size={15} /></button>}>
          <div className="compact-table">
            {transactions.slice(0, 6).map((item) => <div key={item.id}><span className={`transaction-icon transaction-icon--${item.type}`}>{item.type === "buy" ? "+" : "−"}</span><span><strong>{item.ticker}</strong><small>{formatTransactionDate(item.date, item.time)}</small></span><span><strong>{formatBrl(item.total)}</strong><small>{formatNumber(item.quantity, 4)} cotas</small></span></div>)}
          </div>
